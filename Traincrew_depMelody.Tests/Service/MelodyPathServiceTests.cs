...
public class MelodyPathServiceTests
{
    ...
    public void MP001_正常なパス取得()
    {
        ...
        var trainState = TrainStateHelper.CreateAppTrainState("特急");
        _traincrewRepositoryMock.Setup(x => x.GetTrainState()).Returns(trainState);

        var result = _service.GetAudioPath(("館浜", 1), true);
    ...
    public void MP002_デフォルトメロディー()
    {
        ...
        var trainState = TrainStateHelper.CreateAppTrainState("普通");
        _traincrewRepositoryMock.Setup(x => x.GetTrainState()).Returns(trainState);

        var result = _service.GetAudioPath(("存在しない駅", 1), true);
    ...
    public void MP003_上り下りによる変化()
    {
        ...
        var result1 = _service.GetAudioPath(("駒野", 1), true);
        var result2 = _service.GetAudioPath(("駒野", 1), true);
    ...
    public void MP203_館浜2番線_特急()
    {
        ...
        var result = _service.GetAudioPath(("館浜", 2), true);
    ...
    public void MP205_館浜3番線以降()
    {
        ...
        var result = _service.GetAudioPath(("館浜", 3), true);
    ...
    {
        // isMelodyPlaying=true (ON押下直後) → メロディーパス
        var trainState = TrainStateHelper.CreateAppTrainState("普通");
        _traincrewRepositoryMock.Setup(x => x.GetTrainState()).Returns(trainState);
    ...
    {
        // isMelodyPlaying=false (OFF押下直後) → ドア閉め案内パス
        var trainState = TrainStateHelper.CreateAppTrainState("普通");
        _traincrewRepositoryMock.Setup(x => x.GetTrainState()).Returns(trainState);
