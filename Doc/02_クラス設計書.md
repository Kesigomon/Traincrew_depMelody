...
// 注: 駅に在線しているかどうかは、ITrackRepository.IsAnyCircuitAtStationAsync(CurrentCircuitId)
// を使用して、CurrentCircuitIdとstations.csvの情報から動的に計算される
...
public class MelodyControlService : IMelodyControlService
{
    ...
    public async Task StartMelodyAsync()
    {
        ...
        if (!gameState.CurrentCircuitId.Any())
        {
